...
class _DonationDetailState extends State<DonationDetail> {
  ...
  Future<void> fetchPayments() async {
    ...
    for (var doc in snapshot.docs) {
      final data = doc.data();
      final date = DateFormat('MMM d').format((data['date'] as Timestamp).toDate());
      tempChart[date] = (tempChart[date] ?? 0) + (int.parse(data['amount']).toDouble() ?? 0.0);
  ...
  double get totalRaised =>
      payments.fold(0.0, (sum, item) => sum + (int.parse(item['amount']).toDouble() ?? 0.0));

  @override
  Widget build(BuildContext context) {
    final goal = widget.data['targetAmount']?.toDouble() ?? 1000.0;
        ...
        child: Column(
          children: [
            ...
            VolunteeringInfoCard(
              data: widget.data,
              onEdit: () {
                // Navigate to your edit page or show a bottom sheet form
            ...
            Card(
              ...
              child: Padding(
                ...
                child: Column(
                  children: [
                    const Text("Donation Progress", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                    ...
                    Text("Raised: RM ${totalRaised.toStringAsFixed(2)} / RM ${goal.toStringAsFixed(2)}"),
                  ],
                ),
              ),
            ),
            ...
            if (chartData.isNotEmpty)
              Card(
                ...
                child: Padding(
                  ...
                  child: Column(
                    children: [
                      const Text("Daily Contributions", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                            ...
                            titlesData: FlTitlesData(
                              bottomTitles: AxisTitles(
                                sideTitles: SideTitles(
                                  ...
                                  getTitlesWidget: (value, _) {
                                    final date = chartData.keys.elementAt(value.toInt());
                              ...
                              leftTitles: AxisTitles(
                                sideTitles: SideTitles(
                                  showTitles: true,
                                  interval: goal / 5,
                                  getTitlesWidget: (value, _) => Text("RM ${value.toInt()}"),
                                ),
                              ),
                            ),
            ...
            Card(
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    const Text("Donors", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                    const SizedBox(height: 12),
                    ...payments.map((donor) => ListTile(
